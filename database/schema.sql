...
-- 个人记录系统 数据库初始化脚本
-- 数据库：personal_record
-- 版本：v2.5.0（拆分用户端/开发端：新增 admin_user 表、auth_session.user_type、基础数据迁移为全局模板 user_id=0）
-- 表清单：user / admin_user / auth_session / expense_category / expense_record / weight_record
...
-- 预置数据：默认收支分类 14 条 / 默认锻炼动作 9 条 / 默认食物 52 条（均 user_id=0 系统全局模板，所有业务用户共用；
--          用户自定义基础数据 user_id=自己的id，仅本人可见）
-- =====================================================
...
-- =====================================================
-- 2.3 开发账号表（管理员，v2.5.0 拆分）
--    仅开发人员使用，访问开发日志/操作日志/基础数据模板管理；
--    与 user（业务用户）物理隔离，auth_session.user_type=2
-- =====================================================
...
-- =====================================================
-- 4. 预置默认分类（user_id=0 为系统全局模板，所有业务用户共用；用户自定义分类 user_id=自己的id）
--    支出 type=1，收入 type=2
-- =====================================================
...
CREATE TABLE `operation_log` (
  ...
  `user_type`  TINYINT      NOT NULL DEFAULT 1 COMMENT '操作者类型:1=业务用户user表/2=开发账号admin_user表',
  `module`     VARCHAR(20)  NOT NULL COMMENT '操作模块:EXPENSE/WEIGHT/LEARN/USER/NOTE/EXERCISE/FOOD/ADMIN',
...
-- =====================================================
-- 16. 预置默认锻炼动作（user_id=0 为系统全局模板，所有业务用户共用；用户自定义动作 user_id=自己的id）
...
(0, '床上平躺举哑铃', 'strength', 3.5, 17, 90, 1, 1, 1),
(0, '平肩俯卧撑',     'strength', 3.8, 28, 80, 0, 0, 2),
(0, '臀桥',           'strength', 3.8, 30, 60, 0, 0, 3),
(0, '臂力棒',         'strength', 3.5, 25, 90, 1, 0, 4),
(0, '平板支撑',       'plank',    2.8, NULL, NULL, 0, 0, 5),
(0, '散步',           'walk',     0.0, NULL, NULL, 0, 0, 6),
(0, '骑行',           'cycling',  0.0, NULL, NULL, 0, 0, 7),
(0, '爬楼梯',         'stairs',   8.0, NULL, NULL, 0, 0, 8),
(0, '高抬腿击掌',     'cardio',   7.5, 48, 115, 0, 0, 9);
...
-- =====================================================
-- 20. 预置默认食物（user_id=0 为系统全局模板，所有业务用户共用；用户自定义食物 user_id=自己的id）
...
-- 主食
(0, '米饭',       'staple',  116, 2.6, 0.3, 25.9, 2,   0.3, 200, 'g', 1),
(0, '粥',         'staple',   46, 1.1, 0.3,  9.9, 1,   0.1, 250, 'g', 2),
(0, '面条',       'staple',  110, 3.9, 0.4, 22.8, 1,   0.2, 250, 'g', 3),
(0, '馒头',       'staple',  223, 7.0, 1.1, 47.0, 165, 1.3, 100, 'g', 4),
(0, '全麦面包',   'staple',  246,10.1, 3.4, 44.3, 420, 6.8,  35, 'g', 5),
(0, '燕麦',       'staple',  377,10.1, 6.6, 66.7, 4,  10.6,  40, 'g', 6),
(0, '红薯',       'staple',   90, 1.6, 0.2, 20.7, 55,  3.0, 150, 'g', 7),
(0, '玉米',       'staple',  112, 4.0, 1.2, 22.8, 1,   2.9, 150, 'g', 8),
(0, '土豆',       'staple',   77, 2.0, 0.2, 17.2, 6,   2.2, 150, 'g', 9),
-- 肉蛋
(0, '鸡蛋',       'protein', 144,13.3, 8.8,  2.8, 131, 0.0,  50, 'g', 1),
(0, '鸡胸肉',     'protein', 133,24.6, 2.5,  0.6,  34, 0.0, 150, 'g', 2),
(0, '瘦猪肉',     'protein', 143,20.3, 6.2,  1.5,  57, 0.0, 100, 'g', 3),
(0, '牛肉',       'protein', 125,20.2, 4.2,  1.2,  66, 0.0, 100, 'g', 4),
(0, '鱼肉',       'protein', 113,17.9, 4.3,  0.0,  52, 0.0, 150, 'g', 5),
(0, '虾',         'protein',  99,21.2, 0.9,  0.5, 165, 0.0, 100, 'g', 6),
(0, '豆腐',       'protein',  84, 6.6, 3.4,  3.5,   7, 0.4, 100, 'g', 7),
(0, '牛奶',       'protein',  54, 3.0, 3.2,  3.4,  37, 0.0, 250, 'g', 8),
(0, '酸奶',       'protein',  72, 2.5, 2.7,  9.3,  50, 0.0, 200, '杯', 9),
-- 蔬菜
(0, '西兰花',     'veg',      36, 4.1, 0.6,  4.3,  18, 1.6, 150, 'g', 1),
(0, '青菜',       'veg',      15, 1.5, 0.3,  2.2,  73, 1.1, 200, 'g', 2),
(0, '西红柿',     'veg',      20, 0.9, 0.2,  4.0,   5, 1.2, 150, 'g', 3),
(0, '黄瓜',       'veg',      16, 0.8, 0.2,  2.9,   5, 0.5, 150, 'g', 4),
(0, '胡萝卜',     'veg',      39, 1.0, 0.2,  8.8,  69, 2.8, 100, 'g', 5),
(0, '菠菜',       'veg',      28, 2.6, 0.3,  4.5,  85, 1.7, 150, 'g', 6),
(0, '生菜',       'veg',      15, 1.3, 0.3,  2.0,  32, 0.7, 100, 'g', 7),
(0, '冬瓜',       'veg',      12, 0.4, 0.2,  2.6,   3, 0.7, 200, 'g', 8),
-- 水果
(0, '苹果',       'fruit',    53, 0.4, 0.2, 13.7,   2, 1.7, 200, 'g', 1),
(0, '香蕉',       'fruit',    93, 1.4, 0.2, 22.0,   1, 1.2, 100, 'g', 2),
(0, '橙子',       'fruit',    48, 0.8, 0.2, 11.1,   1, 0.6, 200, 'g', 3),
(0, '西瓜',       'fruit',    31, 0.5, 0.3,  6.8,   3, 0.4, 300, 'g', 4),
(0, '葡萄',       'fruit',    45, 0.4, 0.3, 10.3,   2, 0.4, 200, 'g', 5),
(0, '梨',         'fruit',    51, 0.4, 0.2, 13.1,   1, 2.6, 200, 'g', 6),
(0, '蓝莓',       'fruit',    57, 0.7, 0.3, 14.5,   1, 2.4, 100, 'g', 7),
-- 零食饮品
(0, '豆浆',       'snack',    31, 3.0, 1.6,  1.2,   1, 0.6, 250, 'ml', 1),
(0, '可乐',       'snack',    43, 0.0, 0.0, 10.6,   5, 0.0, 330, 'ml', 2),
(0, '坚果混合',   'snack',   645,20.0,54.0, 20.0, 350, 7.0,  30, 'g', 3),
(0, '巧克力',     'snack',   589, 4.3,40.1, 53.4,  24, 1.5,  20, 'g', 4),
(0, '薯片',       'snack',   548, 7.0,37.6, 49.2, 525, 4.0,  30, 'g', 5),
(0, '蛋糕',       'snack',   348, 6.0,13.9, 53.1, 250, 0.6,  80, 'g', 6);

-- =====================================================
-- 20.1 火锅自助餐牛羊肉 + 烧烤预置（v2.4.0 追加）
--     营养值来源：USDA FoodData Central(SR Legacy)、中国食物成分表、
--     中国营养学会/哈医大一院营养科公开口径；均为每100g可食部熟制/市售口径
-- =====================================================
INSERT INTO `food_item` (`user_id`, `name`, `type`, `kcal`, `protein`, `fat`, `carbs`, `sodium`, `fiber`, `default_grams`, `unit_label`, `sort_order`) VALUES
-- 烤肉/煎烤（熟制）
(0, '烤猪肉串',   'protein', 250,20.0,15.0, 10.0, 500, 0.0, 100, 'g', 10),
(0, '煮羊排',     'protein', 341,21.8,27.5,  0.0,  74, 0.0, 120, 'g', 11),
-- 火锅涮肉（牛肉）
(0, '肥牛卷',     'protein', 250,19.0,18.7,  0.8, 233, 0.0, 150, 'g', 12),
(0, '牛百叶(毛肚)', 'protein',  72,14.5, 1.6,  0.0,  61, 0.0, 100, 'g', 13),
(0, '牛肚',       'protein',  72,14.5, 1.6,  0.0,  61, 0.0, 100, 'g', 14),
-- 火锅涮肉（羊肉）
(0, '羊肉卷',     'protein', 203,19.0,14.1,  0.2,  81, 0.0, 150, 'g', 15),
(0, '瘦羊肉片',   'protein', 118,18.0, 4.0,  2.4,  92, 0.0, 100, 'g', 16),
(0, '羊上脑',     'protein',  94,19.0, 2.0,  0.0, 107, 0.0, 150, 'g', 22),
-- 火锅其余牛部位（上脑/里脊/牛腩/牛舌/牛黄喉）
(0, '牛上脑',     'protein', 193,17.4,12.4,  0.0,  60, 0.0, 150, 'g', 17),
(0, '牛里脊',     'protein', 107,22.2, 0.9,  0.0,  54, 0.0, 150, 'g', 18),
(0, '牛腩',       'protein', 332,17.1,29.3,  0.0, 100, 0.0, 150, 'g', 19),
(0, '牛舌',       'protein', 195,17.0,13.3,  2.0,  58, 0.0, 100, 'g', 20),
(0, '牛黄喉',     'protein', 106,17.5, 3.5,  0.8,  48, 0.0, 100, 'g', 21);
