...
const columns: DataListColumn[] = [
  // 时间文本固定 11 个 mono 字符（MM-dd HH:mm），mono 字宽≈0.6em；列宽按 mono 实际宽度算，随响应式字号缩放，各字号下均完整显示const columns: DataListColumn[] = [
  { key: 'time', label: '时间', width: 'calc(7.2 * var(--sk-font-md) + 2 * var(--sk-space-3) + 4px)' },
  { key: 'operator', label: '操作人', width: 110 },
...
const detailRows = computed<DetailRow[]>(() => {
  ...
  return [
    ...
    { key: 'operator', label: '操作人', value: d.operatorName ?? '用户' },
...
<template>
  <PagePanel :title="loading ? '操作日志' : `操作日志 · 共 ${total} 条`" :loading="loading">
    ...
    <DataList :items="logs" :columns="columns" :max-rows="size" :card-below="480" clickable @row-click="onRowClick">
      <template #cell="{ item, column }">
        <span v-if="column.key === 'time'" class="num">{{ formatShortTime(item.createdAt) }}</span>
        <span v-else-if="column.key === 'operator'">{{ item.operatorName ?? '用户' }}</span>
