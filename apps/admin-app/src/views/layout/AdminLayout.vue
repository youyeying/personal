...
/**
 * 开发端布局壳（与用户端 Home.vue 同结构）：
 * - 两栏布局：左侧导航满高 + 右侧（header + 内容区 router-view）
 * - 导航三态：导航栏 / 缩导航栏 / 隐藏（宽度过渡动画）；<600 全屏抽屉
 * - 模块主色随路由匹配（admin 菜单），同步写入 --sk-mod
 * - 挂载时拉取 /auth/me 填充开发账号昵称
 */
...
// 模块主色同步到 html 根：全局滚动条伪元素不继承容器变量，只能读根上的 --sk-mod
watch(
  () => themeStore.module.accent,
  (accent) => {
    document.documentElement.style.setProperty('--sk-mod', accent)
  },
  { immediate: true }
)
