...
/** 开发账号信息（后端 toAdminInfo 脱敏返回，不含密码） */
...
/** 开发账号登录（管理端 /admin/login，admin_user 表，userType=2） */
export function adminLogin(data: { username: string; password: string }) {
  return requestApi<AdminLoginResult>({
...
/** 获取当前登录信息（开发端仅 admin_user 会话；返回 userType 供路由守卫校验） */
export function getMe() {
  return requestApi<{ userType: number; userInfo: AdminInfo }>({
    url: '/auth/me',
    method: 'GET'
  })
}
