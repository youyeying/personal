...
const rangeKey = ref<'1' | '7' | '30' | '90'>('30')
...
function setZoomShow(v: boolean) {
  ...
  chart.ensure()?.setOption({ dataZoom: [{ show: v }] }, { lazyUpdate: true })
}
...
function renderCharts() {
  ...
  const c = palette()
  // 空档兜底：数据点 >30 天才加滑块（默认隐藏，hover 显示），默认视图=最新窗口
  const activeIdx = days.value.map((d, i) => (d.counted ? i : -1)).filter((i) => i >= 0)
  const zoom = trendZoom(c, activeIdx, days.value.length)
  hasZoom.value = zoom.length > 0
  el.setOption({
    grid: { ...TREND_GRID, bottom: zoom.length ? 34 : 24 },
    tooltip: {
      trigger: 'axis',
      formatter: (ps: any) => {
        const p = ps[0]
...
<template>
  <section class="card fds">
    <BlockTitle title="能量结余" hint="1.2×BMR + 当天锻炼净">
      <template #aside>
        <div class="fds__range">
          <button v-for="k in (['1', '7', '30', '90'] as const)" :key="k" class="fds__range-btn" :class="{ 'is-active': rangeKey === k }" @click="rangeKey = k">{{ k === '1' ? '今天' : `近${k}天` }}</button>
        </div>
      </template>
    </BlockTitle>
    ...
    <template v-else>
      ...
      <div class="fds__panel">
        ...
        <div ref="chartEl" class="fds__chart" @mouseenter="onChartEnter()" @mouseleave="scheduleHideSlider()"></div>
      ...
      <div class="fds__panel">
        <div class="fds__panel-head">
          ...
          <span class="fds__panel-hint">日均摄入 vs 每日参考</span>
        </div>
        <div class="fds__dist-list">
      ...
      <div class="fds__panel">
        <div class="fds__panel-head">
          ...
          <span class="fds__panel-hint">周缺口 ÷ 7700 ≈ 预估减脂</span>
        </div>
        <div class="fds__dist-list">
