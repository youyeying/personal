...
async function loadConsume() {
  ...
  try {
    ...
    // 今日结余 = 实际消耗(1.2BMR + 锻炼净) − 已摄入（首页口径，不减目标缺口）
    const budget = base != null ? base + todayExercise : null
    ...
    consumeInfo.value = {
      bmr,
      weekDays,
      weekExercise: roundKcal(weekExercise),
      todayExercise: roundKcal(todayExercise),
      ...
      weekPredictKg,
      todayActual: base != null ? roundKcal(base + todayExercise) : null,
    ...
    <template v-if="overview">
      ...
      <div class="ov-tdee-wrap">
        <template v-if="consumeInfo?.bmr != null">
          ...
          <p class="ov-tdee-hint">左为本周（周一~今天）实际消耗每日平均与预估减脂（缺口 ÷ 7700）· 右为今日结余 = 实际消耗(1.2BMR+锻炼) − 已摄入，负值即已超消耗</p>
        </template>
