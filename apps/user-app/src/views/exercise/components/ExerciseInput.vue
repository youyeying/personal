...
import { walkSpeedKmh, walkMet, cyclingMet, speedMet, stairsMet, repsEffectiveMinutes, calcKcal, strengthWorkKcal, recordNetKcal, totalSeconds, formatDuration, roundKcal, WALK_SPEED_MAX, CYCLING_SPEED_MAX } from '@/utils/exercise'
...
function liveCalc() {
  ...
  switch (item.type) {
    ...
    case 'cardio': {
      ...
      if (reps <= 0) return { met: item.baseMet, minutes: 0, detail: '输入个数', workKcal: 0 }
      // 强度+做功：负重力量（哑铃/臂力棒）速度比定 MET（封顶 2.5×参考，快做不虚高），实际用时做基础，加负重做功
      if (item.type === 'strength' && item.hasWeight) {
        const secTotal = timeSeconds()
        const workKcal = strengthWorkKcal(Number(form.value.weight) || null, reps)
        const minutes = secTotal / 60
        const ref = item.refSpeed ?? 12
        const speed = secTotal > 0 ? reps / (secTotal / 60) : ref
        const ratio = Math.min(speed, ref * 2.5) / ref
        const met = 1 + (item.baseMet - 1) * ratio
        const weightText = form.value.weight ? `${form.value.weight}kg` : '自重'
        const timeText = secTotal > 0 ? ` · 用时 ${formatDuration(secTotal)}` : ''
        return {
          met,
          minutes,
          workKcal,
          detail: `${reps}个 × ${weightText}${timeText} → MET ${met.toFixed(1)} · 做功 ${workKcal.toFixed(2)} kcal（${form.value.weight}kg×${reps}次×0.35m）`
        }
      }
...
const liveKcal = computed(() => {
  ...
  const work = live.value.workKcal ?? 0
  // 负重力量：与 recordNetKcal 同口径（基础不预取整），统一 1 位小数；其余动作维持 calcKcal 取整
  if (work > 0) {
    const baseNet = ((live.value.met - 1) * 3.5 * w * live.value.minutes) / 200
    const baseTotal = (live.value.met * 3.5 * w * live.value.minutes) / 200
    return {
      total: Math.round((baseTotal + work) * 10) / 10,
      net: Math.round((baseNet + work) * 10) / 10
    }
  }
  const base = calcKcal(live.value.met, live.value.minutes, w)
  return { total: base.total, net: base.net }
})
...
const todayTotalKcal = computed(() => roundKcal(todayRecords.value.reduce((s, r) => s + todayKcal(r), 0)))
...
<template>
  <section class="card exi">
    <BlockTitle title="锻炼打卡" :hint="weightKg ? `按体重 ${weightKg}kg 估算` : '请先记录体重'" />
