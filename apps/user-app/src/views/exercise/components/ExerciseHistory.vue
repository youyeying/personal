...
import { recordNetKcal, recordTotalKcal, totalSeconds, formatDuration } from '@/utils/exercise'
...
function calcNet(r: ExerciseRecord): number {
  ...
  const item = itemOf(r)
  if (!item) return 0
  // 统一复用 utils/exercise 的做功主导模型（与输入/分析页同口径，1 位小数）
...
function calcTotal(r: ExerciseRecord): number {
  ...
  const item = itemOf(r)
  if (!item) return 0
  return recordTotalKcal(r, item, w)
}
