...
import { recordNetKcal, recordTotalKcal, roundKcal } from '@/utils/exercise'
...
function computeRow(r: ExerciseRecord, item: ExerciseItem, w: number): KcalRow {
  // 统一复用 utils/exercise 做功主导模型（与历史/输入/分析页同口径，1 位小数）
  return { rec: r, net: recordNetKcal(r, item, w), total: recordTotalKcal(r, item, w), item, date: r.recordDate }
}
...
function sumSince(date: string) {
  // 汇总统一四舍五入到 1 位小数，消除浮点累加尾差（如 2453.2000000003）
  return roundKcal(rows.value.filter((x) => x.date >= date).reduce((s, x) => s + x.net, 0))
}
...
function renderChart() {
  ...
  for (let i = 13; i >= 0; i--) {
    ...
    const s = formatDate(d)
    days.push(s)
    kcal.push(roundKcal(rows.value.filter((x) => x.date === s).reduce((sum, x) => sum + x.net, 0)))
  }
  el.setOption({
...
const dist = computed(() => {
  ...
  for (const x of rows.value) {
    map.set(x.item.name, roundKcal((map.get(x.item.name) ?? 0) + x.net))
  }
  return [...map.entries()]
    ...
    <template v-else>
      ...
      <div class="exs__panel">
        <div class="exs__panel-head">
          ...
          <span class="exs__panel-hint">hover 查看 · 今日青色</span>
        </div>
